...
import { Pencil, Trash2, Check, X, Shirt, Plus, ToggleRight } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import Table from "../../../components/Table";
import {
  getDescuentosPrendasPaginado,
  createDescuentoPrenda,
  updateDescuentoPrenda,
  deleteDescuentoPrenda,
} from "../api/descuentos";
import { getPrendasPaginado } from "../../prenda/api/prendas";
...
const EMPTY = { prendaId: "", porcentaje: "", fechaInicio: "", fechaFin: "", activo: true };

const validate = (f) => {
  const e = {};
  if (!f.prendaId) e.prendaId = "Selecciona una prenda";
...
function DescuentoPrendaModal({ open, onClose, onSubmit, descuento, submitting, prendas }) {
        ...
        >
          ...
          >
            ...
            <h2 className="text-xl font-semibold mb-5 flex items-center gap-2">
              ...
              {descuento ? "Editar Descuento por Prenda" : "Nuevo Descuento por Prenda"}
            </h2>

            <form onSubmit={handleSubmit} className="space-y-4">
              <Field label="Prenda" error={errors.prendaId} required>
                <select value={form.prendaId} onChange={set("prendaId")} className={input(errors.prendaId)} disabled={submitting}>
...
export default function DescuentoPrendaCrud() {
  ...
  const fetchData = async (currentPage = 0) => {
    setLoading(true);
    try {
      const { data } = await getDescuentosPrendasPaginado(currentPage + 1, PAGE_SIZE);
  ...
  const fetchPrendas = async () => {
    try {
      const { data } = await getPrendasPaginado(1, 100);
      setPrendas(Array.isArray(data?.data) ? data.data : []);
    } catch {
      setPrendas([]);
    }
  };
  ...
  const handleSave = async (form) => {
    ...
    try {
      if (editItem) {
        await updateDescuentoPrenda(editItem.id, form);
      } else {
        await createDescuentoPrenda(form);
      }
      closeModal();
      fetchData(page);
    } catch (e) {
  ...
  const handleDelete = async (item) => {
    ...
    const nombre = prenda?.nombre ?? `ID ${item.prendaId}`;
    if (!window.confirm(`¿Eliminar descuento de ${item.porcentaje}% para "${nombre}"?`)) return;
    try {
      await deleteDescuentoPrenda(item.id);
      fetchData(page);
    } catch {
  ...
  const prendaNombre = (id) => {
    const p = prendas.find((pr) => pr.id === id);
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold flex items-center gap-2">
          <Shirt className="h-6 w-6" /> Descuentos por Prenda
        </h2>
        <motion.button
      ...
      <Table animKey={page + "-" + sorted.length}>
        <Table.Header columns={["ID", "Prenda", "%", "Inicio", "Fin", "Activo", "Acciones"]} />
        <Table.Body loading={loading} colSpan={7} empty={sorted.length === 0} emptyText="Sin descuentos por prenda">
          {sorted.map((item) => (
            <motion.tr
              key={item.id} layout
            ...
            >
              ...
              <td className="py-2 px-4 font-medium">{prendaNombre(item.prendaId)}</td>
