...
import { getReseniasPaginado, deleteResenia, moderarResenia } from "./api/resenias";
          ...
          >
            ...
            {resenia && (
              <div className="bg-gray-50 rounded-lg p-3 mb-5 text-sm space-y-1">
                <p><span className="font-medium">Prenda:</span> #{resenia.prendaId}</p>
...
export default function ReseniaCrud() {
  ...
  return (
    <div className="p-6">
      ...
      {/* Filtros */}
      <form onSubmit={handleBuscar} className="flex gap-3 mb-4 flex-wrap">
        <div>
      ...
      <Table animKey={page + "-" + sorted.length}>
        <Table.Header columns={["ID", "Prenda", "Usuario", "Calif.", "Comentario", "Estado", "Moderar", "Acciones"]} />
        <Table.Body loading={loading} colSpan={8} empty={sorted.length === 0} emptyText="Sin reseñas">
          {sorted.map((item) => (
            ...
            >
              ...
              <td className="py-2 px-4">#{item.prendaId}</td>
                ...
                {item.estado === "PENDIENTE" ? (
                  ...
                  <span className="text-xs text-gray-400">
                    {item.estado === "RECHAZADA" && item.motivoRechazo
                      ? <span title={item.motivoRechazo} className="cursor-help underline dotted">Ver motivo</span>
                      : "—"}
                  </span>
                )}
              </td>
