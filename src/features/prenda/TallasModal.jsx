...
export default function TallasModal({ open, onClose, tallas, prendaId }) {
  ...
  const handleSave = async (form) => {
    ...
    try {
      if (form.id) {
        const body = { id: form.id, prendaId: prendaId, tallaId: form.tallaId, stock: form.stock };
        const { data } = await updatePrendaTalla(form.id, body);
        setLocalTallas((prev) =>
          prev.map((t) =>
            t.id === data.object.id
        ...
        const body = { prendaId: prendaId, tallaId: form.tallaId, stock: form.stock };
        const { data } = await createPrendaTalla(body);
        setLocalTallas((prev) => [
          ...prev,
