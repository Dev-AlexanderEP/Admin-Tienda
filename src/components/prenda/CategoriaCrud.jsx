        ...
        >
          ...
          >
            ...
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
              <Tags className="h-6 w-6" /> {categoria ? "Editar Categoría" : "Nueva Categoría"}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="text-sm flex items-center gap-2">
                  <Tags className="h-4 w-4" /> Nombre de Categoría
                </label>
                <input
                  type="text"
                  required
                  name="nomCategoria"
                  value={form.nomCategoria}
                  onChange={handleChange}
                  className="w-full border rounded px-2 py-1"
                  placeholder="Ej: Casacas, Polos, etc."
                />
              </div>
...
export default function CategoriaCrud() {
  ...
  const sortedCategorias = [...categorias].sort((a, b) => a.id - b.id);

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold flex items-center gap-2">
          <Tags className="h-6 w-6" /> Categorías
        </h2>
        <motion.button
